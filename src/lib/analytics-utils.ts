...
export function filterByTimeRange(data: DataPoint[], range: TimeRange): DataPoint[] {
  ...
  switch (range) {
    case "24H":
      // Daily metrics are stored at 1 row/day. 24H reduces to "the
      // latest one or two points". Subtracting 1 day from the latest
      // timestamp keeps yesterday + today (when both exist) so the
      // chart can still draw a 2-point line + a delta vs prior.
